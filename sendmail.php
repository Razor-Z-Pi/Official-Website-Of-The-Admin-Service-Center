...
if (empty($device)) {
    $errors[] = 'Пожалуйста, выберите устройство!!!';
} elseif (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
    $errors[] = 'Выберите из списка устройство!!!';
}
...
// Настройки письма
$to = '>mail@scadmin.ru';
$subject = '=?UTF-8?B?' . base64_encode('Новая заявка с сайта сервисный-центр "Админ"!!!') . '?=';
...
<body>
    <div class="container">
        ...
        <div class="field">
            <div class="label">Тип устройство:</div>
            <div class="value">' . htmlspecialchars($device) . '">' . htmlspecialchars($device) . '</div>
        </div>
        <div class="field">
        ...
        <hr>
        <div class="footer">
            Сообщение отправлено через форму обратной связи сайта mail@scadmin.ru<br>
...
if ($mailSent) {
    ...
    error_log("Ошибка отправки письма с сайта mail@scadmin.ru. Данные: name = $name, email = $email");
